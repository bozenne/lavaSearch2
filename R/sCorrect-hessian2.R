    ...
    for(iP in 1:n.pattern){ ## iP <- 1
        ...
        for(iG in index.mean){ # iG <- 2
            ...
            if(grid.mean[iG,"d2.12"]){
                term1 <- rowSums((d2mu[[iP1]][[iP2]][iIndex,iY,drop=FALSE] %*% iOmegaM1) * iEpsilon)
            }else if(grid.mean[iG,"d2.21"]){
                term1 <- rowSums((d2mu[[iP2]][[iP1]][iIndex,iY,drop=FALSE] %*% iOmegaM1) * iEpsilon)
            }else{
                term1 <- 0
            }
            term2 <- -rowSums((dmu[[iP1]][iIndex,iY] %*% iOmegaM1) * dmu[[iP2]][iIndex,iY])
        ...
        for(iG in index.var){ # iG <- 2
            ...
            term1a <- - diag(iOmegaM1 %*% dOmega[[iP1]][iY,iY,drop=FALSE] %*% iOmegaM1 %*% dOmega[[iP2]][iY,iY,drop=FALSE])
            term2 <- - rowSums((iEpsilon %*% iOmegaM1 %*% dOmega[[iP2]][iY,iY,drop=FALSE] %*% iOmegaM1 %*% dOmega[[iP1]][iY,iY,drop=FALSE] %*% iOmegaM1) * iEpsilon)
        ...
        for(iG in index.hybrid){ # iG <- 1
            ...
            term1 <- - rowSums((dmu[[iP1]][iIndex,iY,drop=FALSE] %*% iOmegaM1 %*% dOmega[[iP2]][iY,iY,drop=FALSE] %*% iOmegaM1) * iEpsilon)
            if(!is.null(dmu[[iP2]]) && !is.null(dOmega[[iP1]])){
                term2 <- - rowSums((dmu[[iP2]][iIndex,iY,drop=FALSE] %*% iOmegaM1 %*% dOmega[[iP1]][iY,iY,drop=FALSE] %*% iOmegaM1) * iEpsilon)
            }else{
                term2 <- 0
            }
