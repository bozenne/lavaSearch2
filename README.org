...
*** Inference: single univariate test (Wald test)
...
two corrections can be used directly by calling the =summary2= method:
#+BEGIN_SRC R :exports both :results output :session *R* :cache no
summary2(e)$coef
#+END_SRC
...
By default both correction are used (i.e. Satterthwaite correction
  with a bias correction for the variance-covariance matrix of the
  coefficients). While this correction is inspired from the
  Kenward-Roger correction in mixed models, the two corrections are
  not exaclty the same and won't, in general, give identical
  results. To use the Satterthwaite corrections alone, set the
  argument =bias.correct= to =FALSE=:

#+BEGIN_SRC R :exports both :results output :session *R* :cache no
summary2(e, bias.correct = FALSE)$coef
#+END_SRC

#+RESULTS:
:       Estimate Std. Error   t-value     P-value df
: Y~X1 0.1550938  0.2032984 0.7628877 0.454435146 20
: Y~X2 0.4581556  0.2025335 2.2621221 0.034980741 20
: Y~~Y 0.5557910  0.1757566 3.1622777          NA  5
: Y    0.7967775  0.2311125 3.4475747 0.002545845 20

Note that for each call to =summary2= the small sample size
correction(s) will be recalculated. However the calculation of the
sample correction(s) can be time consuming. In such a case one can
pre-compute the main terms of the correction (i.e. the derivative of
the variance-covariance matrix) once for all using the =sCorrect=
method (=sCorrect= stands for Satterthwaite correction). When calling
=sCorrect=, the right hand side indicates whether the bias correction
should be used:
#+BEGIN_SRC R :exports both :results output :session *R* :cache no
e2 <- e
sCorrect(e2) <- TRUE ## with biais correction
#+END_SRC

#+RESULTS:

=sCorrect= automatically store the pre-computed terms in the =dVcov=
...
*** Inference: single multivariate test (F-test) 

The function =compare= can be use to perform F-test, i.e. to test
simultaneously several linear combinations of the coefficients.
=compare= uses a contrast matrix to encode in lines which linear
...
#+BEGIN_SRC R :exports both :results output :session *R* :cache no
resTest0 <- compare(e, contrast = resC$contrast, null = resC$null)
resTest0
#+END_SRC
...
data:  
chisq = 26.137, df = 3, p-value = 8.928e-06
sample estimates:
        Estimate   Std.Err        2.5%     97.5%
[Y]    0.7967775 0.2311125  0.34380541 1.2497497
[Y~X1] 0.1550938 0.2032984 -0.24336363 0.5535513
[Y~X2] 0.4581556 0.2025335  0.06119716 0.8551141
#+end_example
...
proposed for the Wald statistic can be applied to the
F-statistic. This is achieved by =compare2=:
#+BEGIN_SRC R :exports both :results output :session *R* :cache no
...
approximation alone:
#+BEGIN_SRC R :exports both :results output :session *R* :cache no
resTest2 <- compare2(e, contrast = resC$contrast, null = resC$null, bias.correct = FALSE)
resTest2
#+END_SRC
...
data:  
F-statistic = 8.7123, df1 = 3, df2 = 20, p-value = 0.0006746
sample estimates:
            Estimate   Std.Err df        2.5%     97.5%
[Y] = 0    0.7967775 0.2311125 20  0.31468536 1.2788697
[Y~X1] = 0 0.1550938 0.2032984 20 -0.26897912 0.5791668
[Y~X2] = 0 0.4581556 0.2025335 20  0.03567805 0.8806332
#+end_example

In this case the F statistic of =compare2= is the same as the
chi-squared statistic divided by the rank of the contrast matrix:
#+BEGIN_SRC R :exports both :results output :session *R* :cache no
...
*** Inference: adjustment for multiple comparisons (Wald test, single model)
...
*** Inference: adjustment for multiple comparisons (Wald test, multiple models)
