### test1-identifyVarCoef.R --- 
##----------------------------------------------------------------------
## Author: Brice Ozenne
## Created: mar 27 2018 (09:50) 
## Version: 
## Last-Updated: mar 27 2018 (10:07) 
##           By: Brice Ozenne
##     Update #: 6
##----------------------------------------------------------------------
## 
...
.identifyVarCoef <- lavaSearch2:::.identifyVarCoef
context("identifyVarCoef")
...
test_that("linear regression - no constrains",{
    ...
    Moment <- conditionalMoment(e.lvm, data = d, param = coef(e.lvm),
                                first.order = FALSE, second.order = FALSE, usefit = TRUE)
    solution <- .identifyVarCoef(e.lvm, Omega = Omega, Moment = Moment)

    expect_equal(solution, coef(e.lvm)[names(solution)], tol = 1e-8)
})

## ** constrains and covariance link
e.lvm <- estimate(lvm(Y1[mu1:sigma]~X1,Y2[mu2:sigma]~X2,Y3~X1+X3,Y2~~Y3), data = d)
...
##
##----------------------------------------------------------------------
### test1-identifyVarCoef.R ends here
